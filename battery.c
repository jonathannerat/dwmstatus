...
int
battery_status(char *output, unsigned int size, const Arg *arg)
{
	GVariant *retval;
	...
	if (proxy == NULL) {
		proxy = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SYSTEM,
			PROXY_FLAGS, NULL, UPOWER_BUS, UPOWER_DISPLAY_DEVICE,
			DBUS_PROPS, NULL, &baterr);
	...
	retval = g_dbus_proxy_call_sync(proxy, "GetAll",
		g_variant_new("(s)", UPOWER_DEVICE_IFACE), CALL_FLAGS, -1, NULL,
		&baterr);
